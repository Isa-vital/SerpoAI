...
const STORAGE_KEY = 'serpo_roadmap_seen_v2';
...
export default function RoadmapModal() {
    ...
    return (
        <div
            className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-black/60 p-2 backdrop-blur-sm sm:items-center sm:p-4"
            onClick={close}
            role="dialog"
            aria-modal="true"
        ...
        >
            <div
                className="relative my-auto flex max-h-[95vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-gradient-to-b from-white to-slate-50 text-slate-900 shadow-2xl ring-1 ring-slate-200"
                onClick={(e) => e.stopPropagation()}
            >
                <button
                    onClick={close}
                    aria-label="Close"
                    className="absolute right-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white/80 text-slate-400 shadow-sm transition-colors hover:bg-slate-100 hover:text-slate-700 sm:right-4 sm:top-4"
                >
                ...
                <div className="min-h-0 flex-1 overflow-y-auto">
                    {/* Header */}
                    <div className="px-4 pb-2 pt-8 text-center sm:px-10 sm:pt-10">
                        <div className="inline-flex items-center justify-center gap-2">
                            <svg className="h-5 w-5 text-indigo-500 sm:h-6 sm:w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l1.6 5.4L19 9l-5.4 1.6L12 16l-1.6-5.4L5 9l5.4-1.6z" /></svg>
                            <h2 id="roadmap-title" className="text-2xl font-extrabold tracking-tight sm:text-4xl">
                                Serpo <span className="bg-gradient-to-r from-indigo-500 to-violet-500 bg-clip-text text-transparent">AI</span>
                            </h2>
                            <svg className="h-4 w-4 text-sky-400 sm:h-5 sm:w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l1.6 5.4L19 9l-5.4 1.6L12 16l-1.6-5.4L5 9l5.4-1.6z" /></svg>
                        </div>
                        <p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500 sm:text-xs sm:tracking-[0.25em]">
                            Our Journey to Transform AI
                        </p>
                    </div>

                    {/* Phases */}
                    <div className="px-3 pb-4 pt-6 sm:px-10 sm:pt-8">
                        ...
                        <div className="space-y-3 md:hidden">
                            {PHASES.map((p) => {
                                const c = COLOR_MAP[p.color];
                                return (
                                    <div key={p.n} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-3">
                                        <div className={`relative flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-full bg-white ring-2 ${c.ring} ${c.glow}`}>
                                            <span className={`text-base font-bold leading-none ${c.text}`}>{p.n}</span>
                                            <span className={`mt-0.5 ${c.text}`}><span className="block [&_svg]:!h-5 [&_svg]:!w-5">{p.icon}</span></span>
                                            {p.n === 1 && (
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <div className={`text-sm font-extrabold leading-tight ${c.text}`}>{p.status}</div>
                                            <p className="mt-1 text-xs leading-snug text-slate-500">{p.text}</p>
                                        </div>
                                    </div>
                                );
                            })}
                    ...
                    {/* Tagline */}
                    <div className="mx-3 mb-4 mt-1 rounded-xl bg-slate-100/80 px-3 py-2.5 text-center text-xs text-slate-600 sm:mx-10 sm:mb-6 sm:mt-2 sm:px-4 sm:py-3 sm:text-sm">
                ...
                <div className="flex shrink-0 items-center justify-center border-t border-slate-200 bg-white px-4 py-3 sm:px-6 sm:py-4">
                    <button
                        onClick={close}
                        className="inline-flex w-full max-w-xs items-center justify-center rounded-lg bg-gradient-to-r from-indigo-500 to-violet-500 px-8 py-2.5 text-sm font-semibold text-white shadow-md transition-transform hover:scale-[1.02] hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-violet-400 focus:ring-offset-2 sm:w-auto"
                    >
                        Okay
                    </button>
                </div>
