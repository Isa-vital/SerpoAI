...
class CapturingTelegram extends TelegramBotService
{
    ...
    { $this->log[] = ['type'=>'msg','text'=>$text]; return ['ok'=>true,'result'=>['message_id'=>1]]; }
    public function sendPhoto(int $chatId, string $photo, string $caption = '', array $replyMarkup = []): array
    { $this->log[] = ['type'=>'photo','caption'=>$caption]; return ['ok'=>true,'result'=>['message_id'=>1]]; }
    public function sendAnimation(int $chatId, string $animation, string $caption = ''): array
    { $this->log[] = ['type'=>'animation','caption'=>$caption]; return ['ok'=>true]; }
...
function evaluate(string $cmd, array $bodies, ?string $error, int $ms): array
{
    ...
    // Failure signals
    if (str_contains($low, 'unable to fetch') || str_contains($low, 'could not fetch')) { $flags[] = 'no_data'; $verdict = 'NO_DATA'; }
    if (str_contains($low, 'not found') || str_contains($low, 'invalid')) { $flags[] = 'invalid_msg'; }
    if (str_contains($low, '❌')) { $flags[] = 'error_emoji'; }
    if (preg_match('/error[: ]/i', $all)) { $flags[] = 'error_word'; }
    if (str_contains($low, 'no data') || str_contains($low, 'not enough data')) { $flags[] = 'no_data'; }
    if (preg_match('/array to string|undefined (index|property|method|variable|key|array key|offset)/i', $all)) { $flags[] = 'php_warning'; }

    // Hallucination markers
    if (preg_match_all('/RSI[^0-9]{0,8}(\d{1,3}(?:\.\d+)?)/i', $all, $m)) {
        foreach ($m[1] as $rsi) { if ($rsi > 100 || $rsi < 0) { $flags[] = "rsi_out_of_range($rsi)"; } }
    }
    if (preg_match_all('/(?:Confidence|confidence)[^0-9]{0,8}(\d{1,3})/i', $all, $m)) {
        foreach ($m[1] as $c) { if ($c > 100) { $flags[] = "confidence>100($c)"; } }
    }
...
foreach ($plan as $category => $cases) {
    foreach ($cases as [$cmd, $desc]) {
        ...
        // Live console line
        fwrite(STDERR, sprintf("[%3d/%d] %-10s %-35s %5dms  %-10s  %s\n",
            $i, $totalCases, $category, mb_substr($cmd,0,35), $ms, $eval['verdict'], implode(',', $eval['flags'])));
    }
}
